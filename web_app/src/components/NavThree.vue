<template>
  <div class="top">
    <div class="navBox">
      ...
      <NavModalThree v-if="showModal" @close="showModal = false" :query="queryTopic" :my-tag="selectTag" :active-index="navActiveIndex"></NavModalThree>
      <!--传递子组件的方法，参数-->
      <!--queryTopic:方法-->
      <!--selectTag：数据-->
...
import NavModalThree from './NavModalThree'
...
export default {
    ...
    methods: {
        queryTopic(data, index) {
            ...
            if (autoScrollInstance) { //确保的插件加载成功
                ...
                console.log(this.$refs.nav.childNodes[index]);
    ...
    mounted() {
        ...
        if (this.navActive == undefined) {
            ...
            // autoScrollInstance.scrollTo(this.$refs.nav.childNodes[this.navActive])
        }
    }
}
</script>
